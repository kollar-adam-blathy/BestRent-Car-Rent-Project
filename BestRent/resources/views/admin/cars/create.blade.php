...
@section('content')
    <div class="container my-5">
        ...
        <form action="/admin/cars" method="POST" enctype="multipart/form-data" class="row g-3">
            @csrf
            ...
            <div class="col-md-4">
                ...
                <input type="number" class="form-control @error('year') is-invalid @enderror" id="year" name="year" value="{{ old('year') }}" min="1980" max="2100" required>
            ...
            <div class="col-md-3">
                ...
                <input type="number" class="form-control @error('seats') is-invalid @enderror" id="seats" name="seats" value="{{ old('seats', 5) }}" min="2" max="9" required>
                @error('seats')<span class="invalid-feedback">{{ $message }}</span>@enderror
            </div>

            <div class="col-md-3">
                <label for="daily_price" class="form-label">Napi ár (Ft)</label>
                <input type="number" class="form-control @error('daily_price') is-invalid @enderror" id="daily_price" name="daily_price" value="{{ old('daily_price') }}" step="1" min="1" required>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select @error('image_type') is-invalid @enderror" id="image_type" name="image_type">
                    <option value="file" {{ old('image_type', 'file') == 'file' ? 'selected' : '' }}>Fájl feltöltés</option>
                    <option value="link" {{ old('image_type') == 'link' ? 'selected' : '' }}>Képlink</option>
                </select>
                @error('image_type')<span class="invalid-feedback">{{ $message }}</span>@enderror
            </div>

            <div class="col-md-4" id="image-file-group">
                <label for="image_file" class="form-label">Képfájl</label>
            ...
            <div class="col-md-4" id="image-link-group">
                ...
                <input type="url" class="form-control @error('image_link') is-invalid @enderror" id="image_link" name="image_link" value="{{ old('image_link') }}" placeholder="https://pelda.hu/auto.jpg">
            ...
            <div class="col-12">
                <button type="submit" class="btn btn-primary"><i class="bi bi-plus-circle"></i> Autó létrehozása</button>
